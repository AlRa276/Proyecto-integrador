...
function initializeModalEvents() {
    ...
    registerForm.addEventListener('submit', async e => {
        ...
        try {
            ...
            if (response.status === 'success' || response.success) {
                ...
                if (redirectUrl) {
                    alert('✅ Cuenta creada. Ahora inicia sesión para continuar.');
                } else {
   ...
   // Reemplaza el listener del modalLoginForm en js/header_footer.js:
    // Dentro de initializeModalEvents(), en el listener del modalLoginForm:
if (modalLoginForm) {
        ...
        try {
            ...
            if (usuario && token) {
                ...
                if (idRol === 1 || nombreRol.includes('ADMIN')) { 
                    ...
                    // Cliente: verificar si hay redirección pendiente
                    if (redirectUrl) {
...
document.addEventListener('click', e => {
    ...
    if (target.closest('.btn-agendar')) {
        ...
        if (!isLoggedIn) {
            // Si NO está logeado, abrir modal de autenticación
            ...
            // Si está logeado, ir directamente a agendar
